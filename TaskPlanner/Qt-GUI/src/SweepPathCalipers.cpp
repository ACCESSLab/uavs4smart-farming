...
//    std::cout<<"[oneshot]: "<< oneshot << "\n";
    if(!oneshot)
        return m_trianlge_decomposition(decomoseArea);
...
std::vector<wykobi::polygon<double, 2>> SweepPathCalipers::m_trianlge_decomposition(const std::vector<wykobi::polygon<double, 2>>& decomoseArea) {
        cout<< "Triangle decomposition "<< endl;
    return decomoseArea;
}
