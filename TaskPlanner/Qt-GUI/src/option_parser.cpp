...
inline
void option_parser(const wykobi::polygon<double, 2> roi, const vector<double>&uav_batteries, const METHOD& method, const string& img_path)
{
...
//    assert(result.size() == uav_batteries.size() && " result does not comply");
    ...
    if(!img_path.empty())
    {
        ...
        height= img.size[0]; width = img.size[1];
